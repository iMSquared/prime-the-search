...
from .utils import INF, get_pairs, merge_dicts, flatten, RED, apply_alpha, default_selector, negate
...
class DistancePRM(PRM):
    ...
    def grow(self, samples):
        old_vertices = list(self.vertices.keys())
...
class DegreePRM(PRM):
    ...
    def grow(self, samples):
        ...
        def wrapped_collsion_fn(value):
            result = self.collision_fn(value, debug=False)
            return result[0]
        ...
        for v1 in new_vertices:
            ...
            for _, v2 in sorted(filter(lambda pair: (pair[1] != v1) and (pair[0] <= self.connect_distance),
                ...
                if v2 not in v1.edges:
                    path = list(self.extend_fn(v1.q, v2.q))[:-1]
                    # Stop as soon as detect collision
                    safe = list(takewhile(negate(wrapped_collsion_fn), default_selector(path)))
                    if len(path) == len(safe):
                        self.connect(v1, v2, path)
                        degree += 1
                else:
                    degree += 1
