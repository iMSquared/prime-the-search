...
def tray_from_arm(arm): # TODO: deprecate
    side = side_from_arm(arm)
    assert (side in ARM_NAMES)
    return '{}_tray'.format(side)
#####################################
...
PR2_GROUPS = {
    ...
    tray_from_arm(LEFT_ARM): ['l_tray_joint'],
    tray_from_arm(RIGHT_ARM): ['r_tray_joint'],
...
PR2_GRIPPER_LINKS = {
    LEFT_ARM: ['l_gripper_palm_link', 'l_gripper_motor_accelerometer_link', 'l_gripper_motor_slider_link',
               'l_gripper_motor_screw_link', 'l_gripper_l_finger_link', 'l_gripper_r_finger_link',
               'l_gripper_l_finger_tip_link', 'l_gripper_r_finger_tip_link'],
    RIGHT_ARM: ['r_gripper_palm_link', 'r_gripper_motor_accelerometer_link', 'r_gripper_motor_slider_link',
                'r_gripper_motor_screw_link', 'r_gripper_l_finger_link', 'r_gripper_r_finger_link',
                'r_gripper_l_finger_tip_link', 'r_gripper_r_finger_tip_link'],
}

PR2_TRAY_ROOTS = {
    LEFT_ARM: 'l_tray_center',
    RIGHT_ARM: 'r_tray_center',
}
...
RETRACT_LEFT_ARM = [0.8, 1.2, 1.5, -2.4, 1.24223229, -1.57, 2.22254125]
RECEPTACLE_LEFT_ARM = [2.1171579596606174, 1.2543767616138641, 0.6977618440489317, -1.3909388008522103, -0.20170870416524878, -1.5061652795814033, -3.001931513440573]
...
def create_tray(robot,arm, visual=True):
    link_name = PR2_TRAY_ROOTS[arm]
    links = get_link_subtree(robot, link_from_name(robot, link_name))
    ## TODO(SJ): set perpendicular tray to the wrist of the robot